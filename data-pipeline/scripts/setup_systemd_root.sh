#!/usr/bin/env bash
# root 로 실행 — 방화벽 포트 개방 + FiftyOne systemd 서비스 등록.
...
echo "=== 1) 방화벽 포트 개방 (5151 FiftyOne) ==="
...
systemctl enable drheri-fiftyone >/dev/null 2>&1
systemctl restart drheri-fiftyone
sleep 6
echo "=== 5) 상태 ==="
systemctl is-active drheri-fiftyone || true
ss -tln | grep -E ":5151" || echo "  (아직 리스닝 전 — 기동 중일 수 있음)"
