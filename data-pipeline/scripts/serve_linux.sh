#!/usr/bin/env bash
# 개발서버(CentOS8, sudo 없음)용 — Dagster(:3333) + FiftyOne(:5151) nohup 기동.
# FiftyOne mongo = 교체된 MongoDB 4.4(AVX 불필요), 검증 끔(min 6.0 우회).
# 접속: 로컬에서  ssh -L 3333:localhost:3333 -L 5151:localhost:5151 jay8126@<host>
set -e
cd "$(dirname "$0")/.."
...
BIND="${BIND:-0.0.0.0}"
# 포트: 이 서버 방화벽이 3000/5000/8090 만 열려 있어 그 중에서 사용 (3333/5151 은 차단됨)
DAGSTER_PORT="${DAGSTER_PORT:-8090}"
FIFTYONE_PORT="${FIFTYONE_PORT:-5000}"

nohup "$PY" -m dagster dev -m drheri_pipeline.definitions --host "$BIND" --port "$DAGSTER_PORT" > "$HOME/dagster.log" 2>&1 &
echo "Dagster  :$DAGSTER_PORT  pid $!  (bind $BIND)"
nohup "$PY" -c "
...
echo "로그: ~/dagster.log  ~/fiftyone.log"
