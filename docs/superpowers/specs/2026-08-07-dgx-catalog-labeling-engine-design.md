...
카탈로그 PDF  (URL 다운로드  또는  //172.30.1.8/nas-metass 파일)
  → 페이지 렌더 (PDF→PNG): 단일 해상도(dpi≈200) 1회 — 검출·VLM·크롭 공통 (§7)
  → [페이지 필터] Grounding DINO 박스 0개면 스킵  ← 느린 VLM 낭비 차단
  → 픽스처 있는 페이지마다 (여러 페이지 병렬):
       Grounding DINO → 픽스처 박스 N개  (렌더 이미지 좌표계, 환산 없음)
       ...
       박스 크롭을 렌더 이미지에서 떠서 DGX 로컬 저장  ← 학습 데이터(dpi 로 크리스프 조절)
...
| `page_renderer` | PDF → 단일 해상도 페이지 이미지 | `(pdf_path, dpi)` → `list[RenderedPage{page_no, image, text}]` |
| `detector` | Grounding DINO 래퍼(모델 1회 로드 재사용) | `RenderedPage.image` → `list[Box]` (score, xyxy) |
...
4. **단일 해상도 렌더**(2026-08-08 개정, 이원화 폐기): 페이지를 하나의 dpi(≈200)로 1회 렌더해
   검출·VLM·크롭에 공통으로 쓴다. 좌표 환산 없음. — 다운스케일은 4개 레버 중 가장 약했고, 유일하게
   VLM 읽기정확도를 **해치는** 역효과가 있어 제거. dpi 하나로 "VLM 가독 ↔ 크롭 크리스프"를 함께 조절
   (200dpi A4≈1654px: 글자 읽히고 픽스처 크롭 ~500px 로 학습에 충분). **승격지점**: 초고해상도 크롭 +
   빠른 소형 VLM 뷰가 동시에 필요해지면 그때 이원화 재도입. GDINO 는 1024 에서 검증했으니 단일 dpi 에서
   threshold 를 DGX 스모크(§9/Task10)에서 재확인.
